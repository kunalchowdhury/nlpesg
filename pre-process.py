...
def index_of_ans(text, answer, start_idx):
    idx=-1
    while idx==-1:
        try:
...
def get_context_answer(result, text, start_idx):
    answers=[]
    contexts=[]
    indexes=[]
    ...
    for ans in answers:
        indexes.append(index_of_ans(text, ans, start_idx))
    return answers, contexts, indexes

def prepare_qs_n_as(text, start_idx):
    ...
    for idx, document in enumerate(tqdm(document_store)):
         ...
         answers, contexts, indexes=get_context_answer(result, text, start_idx)
